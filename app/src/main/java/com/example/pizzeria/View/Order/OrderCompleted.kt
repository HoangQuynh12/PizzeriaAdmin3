...
class OrderCompleted : ComponentActivity() {
        ...
        setContent {
            PizzeriaTheme {
                Surface(
                    ...
                    dbPending.whereEqualTo("status", "Delivered").get().addOnSuccessListener { queryDocumentSnapshot ->
                    ...
                    CompletedScreen(context, pendingList)
                }
            }
        }
    }
...
@Composable
fun CompletedScreen(context: Context, pendingList: List<Order>) {
    Column(
        modifier = Modifier
            .fillMaxSize(),
    ) {
        Row(
            ...
            IconButton(onClick = { context.startActivity(Intent(context, OrderManager::class.java)) }) {
